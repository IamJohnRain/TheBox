...
| 压力静态无动态 | 压力只靠玩家操作改变，无自然衰减/增长 | `interrogation.py:tick()` |
...
| 施压/共情限制 | 混合方案 | 每人每类1次 + 聊天轮次限制（如10轮/人） |
| 时间-压力动态 | 双向动态 | 低压力自然衰减、高压力自然增长，中间段稳定 |
| 心理侧写 | 三级体系 | 初级(Lv.2)/高级(Lv.10)/大师(Lv.15)，对应不同隐藏维度可见 |
| 错误惩罚 | mistake_log 机制 | 错误证据/施压/崩溃导致恐惧下降、时间消耗、评分扣分 |
...
| Phase 2 | 审讯深度 | 反驳机制 + 压力增强 + 证据链 + 主动开口 + 时间-压力动态 + 错误惩罚 | 高 | **9天** |
...
|------|---------|---------|----------|----------|---------|
| `core/suspect_agent.py` | 重构 respond，新增 respond_evidence，恐惧值，多维隐藏指标 | 反驳逻辑，压力效果，时间-压力动态 | — | — | — |
...
| 21 | 压力静态无动态，不操作压力不变 | P1 | 时间-压力双向动态：低压力(0-40)自然衰减-0.5/s，高压力(60+)自然增长+0.3/s，中间段稳定 |
