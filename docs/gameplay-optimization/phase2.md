...
### gameplay_balance.json 新增（Phase 2 配置）

以下字段追加到 `config/gameplay_balance.json`。`core/game_config.py` 只增加访问器或兼容导出，业务代码不得复制这些默认数值。
...
def _check_proactive_speech(self, suspect) -> Optional[dict]:
    ...
    触发条件从 gameplay_balance 配置读取：
    - 默认压力 > 70
    - 默认每 5 轮对话检查一次（而非每秒）
    - 默认概率基于 pressure / 300（约 23%-33%）

    Returns:
        respond 结果字典，或 None（不触发）
    """
    from core.game_config import get_proactive_speech_config

    config = get_proactive_speech_config()
    if suspect.pressure <= config["pressure_threshold"]:
        return None
    if suspect.turn_count % config["turn_interval"] != 0:
        return None

    import random
    probability = suspect.pressure / config["probability_divisor"]
...
### gameplay_balance.json 配置（Phase 1 已定义）

压力/恐惧每轮动态字段已在 Phase 1 写入 `config/gameplay_balance.json`。下面的 Python 片段只表示 `core.game_config` 可提供的兼容字段，默认值不应写死在业务逻辑中。
...
| 包 | 范围 | 允许修改 | 验收重点 |
|----|------|----------|----------|
| 2a 反驳兜底 | `rebuttal_believable`、deception/credibility阈值、反驳事件 | `config/gameplay_balance.json`, `core/game_config.py`, `core/suspect_agent.py`, `core/interrogation.py`, `schemas/events.py`, `tests/test_rebuttal.py` | 高deception提高阈值，pressure>hard阈值强制失败，阈值可由配置覆盖 |
| 2b 证据链 | `chain_with` Schema、chain_bonus、生成提示词 | `config/gameplay_balance.json`, `core/game_config.py`, `core/case_generator.py`, `core/interrogation.py`, `tests/test_evidence_chain.py` | 已出示关联证据时才加bonus，重复出示不叠加，bonus可配置 |
...
| 配置覆盖 | 修改临时配置中的 `rebuttal`、`proactive_speech`、`evidence.chain_bonus` 后，对应行为同步变化 |
...
| 证据链配置覆盖 | 临时配置把 chain_bonus 从默认10改为5后，压力增量按5计算 |
...
| 证据链奖励 | 硬编码`chain_bonus=10` | `config/gameplay_balance.json` 的 `evidence.chain_bonus`，通过 `game_config` 读取 | P2：配置化 |
