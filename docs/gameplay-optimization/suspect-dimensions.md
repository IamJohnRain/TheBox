...
```python
# 影响反驳可信度程序化兜底的阈值
effective_hard_threshold = 80 - int(deception_skill × 0.2)
effective_soft_threshold = 60 - int(deception_skill × 0.1)

# deception=80 → hard=64, soft=52（高压仍能成功反驳）
# deception=50 → hard=70, soft=55
# deception=20 → hard=76, soft=58（很快就无法反驳了）
# deception=5  → hard=79, soft=59.5（几乎无法反驳）
```
...
| 2 | 出示正确物证 | 62 | 40 | 36 | 58 | 38 | 40 | fear+5, pressure+12, 各维度自然联动 |
...
| 8 | 出示正确书证(证据链) | 60 | 45 | 36 | 55 | 37 | **56** | fear+5, chain_bonus+10 |
| 9 | 等待(3秒) | 58 | 44 | 37 | 55 | 37 | **57** | fear自然冷却, 各维度联动 |
