...
| 触发 | 变化 | 条件 |
|------|------|------|
| 出示正确证据 | +基础增量×fear系数×defiance系数 | evidence.related_suspect == suspect.name；基础值 physical/document/testimony = 18/12/9 |
...
| 每轮动态（低段） | -1/轮 | pressure < 30 |
| 每轮动态（高段） | +1/轮 | pressure >= 70 |
